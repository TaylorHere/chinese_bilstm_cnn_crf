...
def test_cosineconvolution_2d(border_mode, subsample, use_bias_mode, use_regularizer):
    num_samples = 2
    num_filter = 2
    stack_size = 3
    num_row = 10
...
def test_cosineconvolution_2d_correctness():
    ...
    model.add(
        CosineConvolution2D(
            1, (5, 5), use_bias=True, input_shape=input_dim, data_format=data_format
        )
    )
    ...
    model.add(
        CosineConvolution2D(
            1, (5, 5), use_bias=False, input_shape=input_dim, data_format=data_format
        )
    )
    model.compile(loss="mse", optimizer="rmsprop")
    W = model.get_weights()
