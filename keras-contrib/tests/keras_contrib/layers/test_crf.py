...
@pytest.mark.xfail(
    is_tf_keras, reason="TODO: fix it. Using K.tf which is bad.", strict=True
)
def test_CRF():
    # data
    ...
    assert_allclose(y_pred[0, -4:], 1.0 / output_dim, atol=1e-6)
    assert_allclose(y_pred[1, :5], 1.0 / output_dim, atol=1e-6)
