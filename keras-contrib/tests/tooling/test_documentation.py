...
def handle_function(name, member):
    ...
    if doc is None:
        raise ValueError(
            "{} function doesn't have any documentation".format(name),
            member.__module__,
            inspect.getmodule(member).__file__,
        )
...
def assert_doc_style(name, member, doc):
    ...
    if first_line.strip()[-1] != ".":
        raise ValueError(
            "{} first line should end with a '.'".format(name), member.__module__
        )
...
def assert_function_style(name, member, doc, args):
    ...
    if has_return and "# Returns" not in doc:
        ...
        if len(return_in_sub) < len(has_return):
            raise ValueError(
                "{} needs a '# Returns' section".format(name), member.__module__
            )
    ...
    if has_raise and "# Raises" not in doc:
        ...
        if len(raise_in_sub) < len(has_raise):
            raise ValueError(
                "{} needs a '# Raises' section".format(name), member.__module__
            )

    if len(args) > 0 and "# Arguments" not in doc:
        raise ValueError(
            "{} needs a '# Arguments' section".format(name), member.__module__
        )
