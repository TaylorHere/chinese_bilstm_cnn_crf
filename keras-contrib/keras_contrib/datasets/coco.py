...
def class_weight(
    image_segmentation_stats_file=None, weighting_algorithm="total_pixels_p_complement"
):
...
def coco_config():
    ...
    data_prefixes = ["train2014", "val2014", "test2014", "test2015"]
    ...
    annotation_paths = [
        os.path.join(dataset_path, postfix) for postfix in annotation_json
    ]
    # only first two data prefixes contain segmentation masks
    seg_mask_image_paths = [
        os.path.join(dataset_path, prefix) for prefix in data_prefixes[0:1]
    ]
    seg_mask_output_paths = [
        os.path.join(seg_mask_path, prefix) for prefix in data_prefixes[0:1]
    ]
    ...
    voc_imageset_txt_paths = [
        os.path.join(dataset_path, "annotations", prefix + ".txt")
...
@data_coco.command
def print_coco_files(
    dataset_path, filenames, dataset_root, urls, md5s, annotation_paths
):
...
@data_coco.command
def coco_to_pascal_voc_imageset_txt(
...
def coco_setup(
    ...
    coco_to_pascal_voc_imageset_txt(
        voc_imageset_txt_paths, image_dirs, image_extensions
    )
