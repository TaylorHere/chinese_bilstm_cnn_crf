...
DENSENET_121_WEIGHTS_PATH = (
    r"https://github.com/titu1994/DenseNet/releases/download"
...
def DenseNet(
    ...
    if weights == "imagenet":
        ...
        if (
            (depth == 121)
            and (nb_dense_block == 4)
            and (growth_rate == 32)
            and (nb_filter == 64)
            and (bottleneck is True)
            and (reduction == 0.5)
            and subsample_initial_block
        ):
            if include_top:
...
def DenseNetFCN(
    ...
    if type(nb_layers_per_block) is not list and nb_dense_block < 1:
        raise ValueError(
            "Number of dense layers per block must be greater than 1. "
...
def __conv_block(
    ...
    with K.name_scope("ConvBlock"):
        ...
        x = BatchNormalization(
            axis=concat_axis, epsilon=1.1e-5, name=name_or_none(block_prefix, "_bn")
        )(ip)
        x = Activation("relu")(x)
        ...
        if bottleneck:
            ...
            x = Conv2D(
                ...
                name=name_or_none(block_prefix, "_bottleneck_conv2D"),
            )(x)
            x = BatchNormalization(
                axis=concat_axis,
                epsilon=1.1e-5,
                name=name_or_none(block_prefix, "_bottleneck_bn"),
            )(x)
            x = Activation("relu")(x)
        ...
        x = Conv2D(
            ...
            padding="same",
            use_bias=False,
            name=name_or_none(block_prefix, "_conv2D"),
        )(x)
        if dropout_rate:
...
def __transition_up_block(
    ip, nb_filters, type="deconv", weight_decay=1e-4, block_prefix=None
):
    ...
    with K.name_scope("TransitionUp"):
        ...
        if type == "upsampling":
            ...
            x = SubPixelUpscaling(
                scale_factor=2, name=name_or_none(block_prefix, "_subpixel")
            )(x)
            x = Conv2D(
                nb_filters,
                (3, 3),
            ...
            x = Conv2DTranspose(
                ...
                kernel_initializer="he_normal",
                kernel_regularizer=l2(weight_decay),
                name=name_or_none(block_prefix, "_conv2DT"),
            )(ip)
        return x
...
def __create_dense_net(
    ...
    with K.name_scope("DenseNet"):
        ...
        if subsample_initial_block:
            x = BatchNormalization(axis=concat_axis, epsilon=1.1e-5, name="initial_bn")(
                x
            )
            x = Activation("relu")(x)
...
def __create_fcn_dense_net(
    ...
    with K.name_scope("DenseNetFCN"):
        ...
        if include_top:
            x = Conv2D(
                nb_classes, (1, 1), activation="linear", padding="same", use_bias=False
            )(x_up)
